...
def get_sa_model(weight_sa_file):
  model_type='sa'
  import keras
  ...
  model.load_weights(weight_sa_file,by_name=True)
  return model




def get_ss_model(weight_ss_file):
  model_type='ss'
  import keras
  from keras.models import Model
  from keras.layers.convolutional import Conv1D
  ...
  model=Model(inputs=[input0],outputs=[conv])
  model.compile(optimizer='adam',loss='categorical_crossentropy',metrics=['accuracy'])
  model.load_weights(weight_ss_file,by_name=True)
  return model
...
def primateAI_model(weight_ss_file, weight_sa_file):
  import keras
  ...
  input2 = Input (shape=(seq_length,20),name='conservation_primates')
  input3 = Input (shape=(seq_length,20),name='conservation_mammals')
  ...
  conv_consrv_primate=Conv1D(L, 1, name='1d_conv_primates')(input2)
  conv_consrv_mammals=Conv1D(L, 1, name='1d_conv_mammals')(input3)
  conv_consrv_vertebrates=Conv1D(L, 1, name='1d_conv_vertebrates')(input4)
  conv_orig=keras.layers.add([conv_orig,conv_consrv_primate,conv_consrv_mammals,conv_consrv_vertebrates,struc,solv],name='merge_orig_conserv')
  conv_snp=keras.layers.add([conv_snp,conv_consrv_primate,conv_consrv_mammals,conv_consrv_vertebrates,struc,solv],name='merge_snp_conserv')
  ...
  model=Model(inputs=[input0,input1,input2,input3,input4],outputs=[output0])
